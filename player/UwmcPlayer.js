...
class UwmcPlayer extends Player {
    ...
    get daysSinceLastPlayed() {
        return Math.floor( Math.abs( this.getLastPlayed().getTime() - new Date().getTime() ) / ( 1000 * 3600 * 24 ) )
    }
     ...
     */
    get active() {
        return this.getDaysSinceLastPlayed() < 62
    }

    /*
     * saves the data of this player to the database or updates it if there is already a player with this uuid in the database
     */
    saveToDb( db ) {
        let player = this;
